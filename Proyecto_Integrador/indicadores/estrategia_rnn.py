...
def ejecutar(df, features_list=None, lookback=10, epochs=30, batch_size=32, model_save_path=None):
    """
    Modelo de Red Neuronal Recurrente (RNN) para predecir la dirección del mercado.
    Alinea predictoras (lagged t-1), escala, genera secuencias con lookback,
    entrena la red y genera señales de compra/venta para el set de prueba.
    """
    ...
    # Prepare test set with lookback padding to align predictions
    ...
    rnn_model.add(SimpleRNN(32, input_shape=(lookback, len(lagged_features)), activation='relu'))
    ...
    if model_save_path:
        ...
        print(f"Red Neuronal Recurrente guardada en: {model_save_path}")
